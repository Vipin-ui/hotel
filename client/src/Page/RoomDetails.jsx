...
const RoomDetails =()=> {
    ...
    const checkAvailability = async () => {
      try {
        ...
        const { data } = await axios.post(
          'http://localhost:3000/api/booking/check-availability',
          {
            room: id,
            checkInDate,
            checkOutDate,
        ...
        toast.error(error.response?.data?.error || error.message || 'Failed to check availability')
        setIsAvailable(false)
      }
    }
    ...
    useEffect(()=>{
     const found = rooms.find(room =>room._id === id)
        if(found){
          setRoom(found)
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-2">
        ...
        <div className="flex flex-col items-start gap-4">
          <div className="flex gap-4">
              <img
                src={room?.hotel?.owner?.image || assets.profile}
              alt="Host"
